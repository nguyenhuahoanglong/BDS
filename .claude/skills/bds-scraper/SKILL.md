...
```
User criteria → Build search URL → Chrome scrape listings (incl. VIP/seller signals)
    → Google Maps coords → score_listings.py (trust + ±25% outlier flag)
...
| 2 | Scrape listings + VIP/seller signals via Chrome | `mcp__claude-in-chrome__*` |
| 2.5 | Score trust + flag price outliers | `scripts/score_listings.py` |
...
**Details for steps 1–5 (incl. 2.5)**: see `references/workflow-detailed.md`.
...
| `score_listings.py` | Enricher — adds `trust_score`, `price_per_m2`, `price_confidence` (±25% outlier flag) | `python score_listings.py --data projects_raw.json --output projects.json` |
| `generate_excel.py` | Transformer — projects JSON → formatted xlsx (sorted by trust) | `python generate_excel.py --data projects.json --output out.xlsx` |
...
- `scripts/score_listings.py` — JSON → JSON (adds trust + price confidence)
